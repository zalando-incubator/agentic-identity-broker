...
5. **Given** an active CIMD confidential service, **When** the authorization server of its registered third-party OAuth2 service retrieves its metadata document, **Then** the response is `200 OK`, uses `application/json`, contains the required document fields, and is cacheable for no longer than five minutes.
6. **Given** an unknown, inactive, deleted, public, or static confidential service identifier, **When** a caller requests its CIMD metadata document or public-key document, **Then** the broker responds with `404 Not Found` and no client metadata or key material.
...
An operator rotates the broker signing key used for confidential CIMD authentication. The authorization servers of registered third-party OAuth2 services can discover the newly usable public key before receiving assertions signed with it, so user sessions continue to connect and refresh without an avoidable authentication outage.
...
**Independent Test**: Rotate the broker signing key, retrieve the public keys before the new key is used, and complete a CIMD confidential token request that the provider validates successfully.

**Acceptance Scenarios**:

1. **Given** a CIMD confidential service, **When** an operator rotates the broker signing key, **Then** the metadata's public-key reference remains stable and the newly usable verification key is available before new assertions use it.
...
3. **Given** no usable signing key is available, **When** a CIMD confidential token request is needed, **Then** the broker fails closed and does not send an unsigned, stale, or differently authenticated request.
...
- What happens when public keys are rotated? The advertised key location remains stable, only public verification keys are published, and previously active keys remain available for the existing rotation overlap period.
- What happens when a service is deleted? Its broker-hosted metadata document must no longer identify it as an active client.
- What happens when the broker cannot initialize mode-independent CIMD signing material while CIMD confidential services are configured? The broker refuses startup in that configuration. When no CIMD confidential service is configured, normal startup remains available; any later CIMD confidential-service registration or use fails closed until usable signing material and public-key publication are available.
...
- **FR-003**: A service configured with `private_key_jwt` MUST be registrable and updatable without a client secret. The broker MUST use its internally generated, mode-independent, broker-global private signing-key set as the confidential-client credential and MUST NOT accept or store a shared client secret for that service.
...
- **FR-008**: For every active CIMD confidential service, the broker MUST anonymously serve a Client ID Metadata Document from its stable client identifier URL over HTTPS.
...
- **FR-012**: A metadata document for an inactive, deleted, public, or static confidential service MUST NOT identify the broker as an active CIMD confidential client.
...
- **FR-017**: A signed assertion used for CIMD confidential authentication MUST use only a currently usable private key from the mode-independent, broker-global signing-key set retained inside the broker. Its public counterpart MUST be available from the metadata document's public-key URL before the assertion is used; the private key MUST never be accepted from an operator or published.
...
- **API-003**: The public contract MUST document the anonymously retrievable metadata document and public-key response for active CIMD confidential services, including response semantics and the fact that they disclose public information only.
...
- **API-005**: The public metadata contract MUST use `GET /.well-known/oauth-client/{service-id}` as the stable client identifier URL. For an active CIMD confidential service it returns the fields and HTTP semantics in FR-023; for every other service state it returns the `404 Not Found` semantics in FR-025.
- **API-006**: The public-key contract MUST use `GET /.well-known/oauth-client/{service-id}/jwks.json`. For an active CIMD confidential service it returns the JWK Set and HTTP semantics in FR-024; for every other service state it returns the `404 Not Found` semantics in FR-025.
...
- **SC-002**: 100% of metadata retrieval checks for an active CIMD confidential service return a document whose identifier matches the retrieval URL, declares `private_key_jwt`, includes a callback URI and public-key reference, and exposes no private material.
...
- **SC-008**: Under normal operating load, 95% of metadata and public-key retrievals complete in under one second.
...
- The broker's signing-key lifecycle for CIMD confidential authentication is mode-independent and broker-global: it generates and protects the shared internal private signing-key set in `proxy`, `local`, and `hybrid` modes. It is distinct from a shared `client_secret`, which is neither accepted nor stored for an individual service; only the public verification keys are published.
- The user request selects `private_key_jwt` as the one supported asymmetric CIMD authentication method. Other key-based methods remain out of scope; this selection is grounded in the CIMD draft's `private_key_jwt` example and in the broker's existing ES256 signing-key lifecycle.
...
- The broker's existing signing-key and public-key publication lifecycle: it provides the public keys and signing capability that make secret-free confidential authentication possible.
