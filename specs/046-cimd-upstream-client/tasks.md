...
- [X] T005 [P] Write semantic-red migration-033 and public-JWK regression coverage in `tests/integration/migrations/migrations_test.go`, `tests/integration/infra/oauth2_signing_key_bootstrap_test.go`, and `tests/e2e/aggregated_jwks_test.go`: backfill legacy signing keys, prove `033` apply/replay and its CIMD-key rollback guard, and assert that `/oauth2/jwks.json` keeps its existing token/upstream mode-appropriate contents and contains no CIMD-domain keys.
...
- [X] T008 Add `migrations/033_add_cimd_key_domain.up.sql` and `migrations/033_add_cimd_key_domain.down.sql`: backfill `token_signing`, add a closed domain check, replace the global active-current index with a per-domain active-current index, and refuse rollback while any `cimd_client_authentication` row exists.
...
- [X] T022 [P] Add semantic-red migration-034 cases in `tests/integration/migrations/migrations_test.go` for legacy static/public rows, valid secretless `private_key_jwt` rows, apply/replay, and the guarded rollback that refuses to discard CIMD service authentication state.
- [X] T023 Verify the completed `migrations/033_add_cimd_key_domain.up.sql` and `migrations/033_add_cimd_key_domain.down.sql` from T008 against `specs/046-cimd-upstream-client/data-model.md`: confirm the backfill, closed domain check, per-domain active-current index, global `kid` preservation, and CIMD-key rollback guard; do not create a second migration in this phase.
- [X] T024 Add `migrations/034_add_cimd_private_key_jwt_authentication.up.sql` and `migrations/034_add_cimd_private_key_jwt_authentication.down.sql`; allow `private_key_jwt` only when `client_secret_encrypted IS NULL`, and refuse rollback while any service has `token_endpoint_auth_method = private_key_jwt`.
...
- [X] T074 [P] Verify Principles IX and XIII against `migrations/033_add_cimd_key_domain.up.sql`, `migrations/033_add_cimd_key_domain.down.sql`, `migrations/034_add_cimd_private_key_jwt_authentication.up.sql`, `migrations/034_add_cimd_private_key_jwt_authentication.down.sql`, and the four `tests/e2e/thirdparty_cimd_*_test.go` files; all migrations and all 31 scenario mappings must be present.
