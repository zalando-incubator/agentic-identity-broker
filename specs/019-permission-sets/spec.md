...
When a user visits the consent screen for an agent, the screen displays the agent's requested permission sets prominently at the top — using human-readable names and descriptions — independent of how many third-party services those sets span. Required service connections appear below as secondary action items, but only for services where the user does not already have an active session.

**Why this priority**: This is the primary UX improvement. Users should understand what capability they are granting (a named permission set) before seeing which technical connections are needed. Hiding already-active service sessions reduces noise and cognitive load.

**Independent Test**: Can be fully tested by rendering the consent screen for an agent with at least two permission set alternatives and one service that already has an active session, then verifying the permission set cards appear first and the already-active service's connect button is absent.
...
3. **Given** the user already has an active OAuth2 session for a service shown in the connections section, **When** the consent screen renders, **Then** no action item (connect button) is shown for that service — it appears as already connected with a satisfied indicator.
...
- **SC-003**: The consent screen displays zero service connection action items for services where the user already has an active session — verified across all acceptance scenarios.
...
- Q: How are service connections displayed — all services, only mandatory, or with disclosure? → A: Option A — all services from `service_requirements` lacking an active session, with "Required"/"Optional" badge per `requirement_type`; already-connected services show a satisfied indicator with no action button.
