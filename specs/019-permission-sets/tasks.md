...
- [ ] T040 [US2] Restructure `web/src/pages/AgentGrantDetailPage.tsx`: render `<PermissionSetsList>` first, then **dynamic** service connections section; service connections computed as union of: (1) all mandatory SR services (always shown), (2) optional SR services that appear in `includedServiceIds` of any selected PS; filter each to show connect button only for services lacking active sessions; **Approve button disabled** (FR-020) until all displayed services have active sessions; wire structured `granted_permission_sets: {ps_id: [included_service_ids]}` into consent submission payload
- [ ] T041 [US2] Verify Playwright tests from T011 now pass: all 8 UI scenarios — PS section above services, mandatory PS locked, optional PS toggles, per-service toggles within PS card, dynamic service connections on PS toggle, satisfied service shows no connect button, Approve blocked when unconnected, Approve enabled when all connected; screenshots captured
...
**Checkpoint**: User Story 2 complete — consent screen shows PSets first, active sessions hide connect buttons, Playwright tests green, E2E US2 scenarios green
