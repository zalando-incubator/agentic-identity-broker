...
Use transaction-scoped PostgreSQL advisory locks. Lifecycle and subject use separate two-int key classes, distinct from existing signing bootstrap locks. Subject key hashes may collide; this only reduces concurrency because predicates still use exact subject equality. A delivery candidate's subject may be looked up without a barrier only for lock selection: reload the event and pending row after acquiring the gates and skip if either disappeared. `FOR UPDATE SKIP LOCKED` on the pending row lets multiple workers compete without duplicate concurrent dispatch. No event payload survives release of its dispatch transaction.
...
Returns a bigint count of deleted event rows. Reject null/empty selection. One invocation/transaction acquires lifecycle shared then subject exclusive, deletes associated pending rows and all matching event rows across partitions, and returns the count. Repeat returns zero successfully. The function is VOLATILE so statements after waiting for locks see committed predecessors; do not capture the deletion snapshot before acquiring the barrier. It cannot return success before commit (clients using explicit BEGIN must COMMIT before reporting completion).
...
Operator/scheduler interface:

```sql
SELECT public.business_event_maintain_partitions();
```

Run using migration-owned credentials, not runtime credentials. This SECURITY INVOKER function obtains lifecycle exclusive, reads one validated policy value, creates the current and next seven days of six-hour UTC partitions, and drops eligible event/pending partition pairs transactionally. Pairs are created through `public.business_event_create_partition_pair(lower_bound timestamptz)`, a migration-owned SECURITY INVOKER function that rejects non-six-hour UTC boundaries, derives quoted names, and creates the pair idempotently; PUBLIC and runtime execution are revoked. Maintenance uses it for current and future windows; PostgreSQL retention tests call it as the migration owner to seed historical windows. Eligibility is `partition_upper_bound <= database_now - retention`. Capture database-now once per sweep. Dynamic identifiers are generated from trusted timestamps and quoted. Missing metadata, policy corruption, or privilege errors abort without partially dropping a pair.

The initial migration and pre-install/pre-upgrade migration job invoke the function before broker readiness. The PostgreSQL-only CronJob runs `*/5 * * * *`, `concurrencyPolicy: Forbid`, and calls it with `psql -v ON_ERROR_STOP=1`. Reuse the existing `migration.grants.image`, migration service account, migration Secret, security context and database connection pattern. No runtime DDL, migration binary in the broker image, or new helper binary is needed. An external deployment must install the same scheduled SQL operation under its operations scheduler. An advisory lock also protects against another deployment or manual maintenance invocation.
...
| Pending-reference DML | Yes | No | Through erasure function | Yes |
| Policy update | Validated startup DML under lifecycle lock | No | No | Yes |
| CREATE/DROP partitions | No | No | No | Yes |
| Maintenance and partition-pair functions | No | No | No | Yes |
