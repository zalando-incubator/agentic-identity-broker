...
import (
	...
	"github.com/agentic-identity-broker/sample-agent/internal/server"
)
...
func main() {
	...
	if len(os.Args) > 1 {
		...
		// Try current directory first (when running from mocks/sample-agent)
		if _, err := os.Stat("config.yaml"); err == nil {
			configDir = "."
		} else {
			// Fall back to root-relative path (when running from project root)
			configDir = "mocks/sample-agent"
		}
	}
